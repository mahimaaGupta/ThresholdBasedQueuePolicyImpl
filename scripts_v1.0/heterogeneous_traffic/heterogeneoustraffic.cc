...
static void SinkRxCount8(Ptr<const Packet> p, const Address &ad ){  
  pkt_count8++;}
static void HtttpTxCount(Ptr<const Packet> p ){  
  Http_data_count = Http_data_count + + p->GetSize();}
static void HtttpTxCount2(Ptr<const Packet> p ){  
  Http_data_count2 = Http_data_count2 + + p->GetSize();}
static void HtttpTxCount3(Ptr<const Packet> p ){  
  Http_data_count3 = Http_data_count3 + + p->GetSize();}
static void HtttpTxCount4(Ptr<const Packet> p){  
  Http_data_count4 = Http_data_count4 + + p->GetSize();}
static void HtttpTxCount5(Ptr<const Packet> p ){  
  Http_data_count5 = Http_data_count5 + + p->GetSize();}
static void HtttpTxCount6(Ptr<const Packet> p){  
  Http_data_count6 = Http_data_count6 + + p->GetSize();}
static void HtttpTxCount7(Ptr<const Packet> p ){  
  Http_data_count7 = Http_data_count7 + + p->GetSize();}
static void HtttpTxCount8(Ptr<const Packet> p ){  
...
StartTracingSink(){
    for(int i = 0; i < 5 ; i++){
      ...
      if (i == 4)
        Config::ConnectWithoutContext("/NodeList/" + std::to_string(i+ (i+1)) + "/ApplicationList/*/$ns3::ThreeGppHttpServer/Tx", MakeCallback(&HtttpTxCount));
    }
    ...
    for(int i=5; i < 10; i++){
      ...
      if (i == 9)
        Config::ConnectWithoutContext("/NodeList/" + std::to_string(i+ (i+1)) + "/ApplicationList/*/$ns3::ThreeGppHttpServer/Tx", MakeCallback(&HtttpTxCount2));
    }
    ...
    for(int i=10; i < 24; i++){
      ...
      if(i == 23)
        Config::ConnectWithoutContext("/NodeList/" + std::to_string(i+ (i+1)) + "/ApplicationList/*/$ns3::ThreeGppHttpServer/Tx", MakeCallback(&HtttpTxCount3));
    }
    ...
    for(int i=24; i < 29; i++){
      ...
      if(i==28)
        Config:: ConnectWithoutContext("/NodeList/" + std::to_string(i+ (i+1)) + "/ApplicationList/*/$ns3::ThreeGppHttpServer/Tx", MakeCallback(&HtttpTxCount4));
    }
    ...
    for(int i=29; i < 32; i++){
      ...
      if(i == 31)
        Config::ConnectWithoutContext("/NodeList/" + std::to_string(i+ (i+1)) + "/ApplicationList/*/$ns3::ThreeGppHttpServer/Tx", MakeCallback(&HtttpTxCount5));
    }
    ...
    for(int i=32; i < 53; i++){
      ...
      if(i == 52)
        Config::ConnectWithoutContext("/NodeList/" + std::to_string(i+ (i+1)) + "/ApplicationList/*/$ns3::ThreeGppHttpServer/Tx", MakeCallback(&HtttpTxCount6));
    }
    ...
    for(int i = 53; i < 57; i++){
      ...
      if(i == 56)
        Config::ConnectWithoutContext("/NodeList/" + std::to_string(i+ (i+1)) + "/ApplicationList/*/$ns3::ThreeGppHttpServer/Tx", MakeCallback(&HtttpTxCount7));
    }
    ...
    for(int i=57; i < 60; i++){
      ...
      if(i == 59)
        Config::ConnectWithoutContext("/NodeList/" + std::to_string(i+ (i+1)) + "/ApplicationList/*/$ns3::ThreeGppHttpServer/Tx", MakeCallback(&HtttpTxCount8));
    }    
}
...
{
  int UDP_Source_number[8] = {4, 9, 23, 28, 31, 52, 56, 59}; int itr = 0;
  for(int i=0; i < number_of_sources - 8; i++){
...
main (int argc, char *argv[])
{
    ...
    for(uint32_t i = 0; i < 8 ; i++){
      ...
      for(uint32_t j = starting_index; j < last_index; j++){
        src_socket[itr] = Socket::CreateSocket (nodes.Get(j * 2), TypeId::LookupByName ("ns3::TcpSocketFactory"));
    ...
    for(uint32_t i = 0; i < number_of_sources - 8 ; i++){
        ...
        Simulator::Schedule (Seconds (gap),&StartFlow, src_socket[i], Ipv4Address(dstaddr[i].c_str()), dstport);
    }
    
    for(int i = 1 ; i < simDuration ; i++){
        for(int j = 0 ; j < number_of_sources - 8 ; j++){
            double gap = expRandomVariable->GetValue();
            Simulator::Schedule (Seconds (i + gap),&StartFlow2, src_socket[j], Ipv4Address(dstaddr[j].c_str()), dstport);
        }
    }
  ...
  for (int  i = 0; i < number_of_UDP_sources ; i++){
    UDPApps[i] = UDPSink.Install (nodes.Get (UDP_sources_number[i] + (UDP_sources_number[i]+1))); 
    UDPApps[i].Start (Seconds (0.0));
    UDPApps[i].Stop (Seconds (simDuration));
    AddressValue remoteAddress (InetSocketAddress (ip_addresses[UDP_sources_number[i] + number_of_sources + 15].GetAddress (1), port));
  ...
  for (int  i = 0; i < number_of_HTTP_Servers; i++){
    Address serverAddress = Address (ip_addresses[UDP_sources_number[i] + number_of_sources + 15].GetAddress (1));
    
    ThreeGppHttpServerHelper serverHelper (serverAddress);
    serverApplications[i]= serverHelper.Install (nodes.Get (UDP_sources_number[i] + (UDP_sources_number[i] + 1)));
