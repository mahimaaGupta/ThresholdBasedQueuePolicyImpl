...
def calculate_QueueingDelay(f1, QueueType, RTT, Metric):
    ...
    filename = "./HeteroResults/" + QueueType + "/" + QueueType +  RTT + "RTT" + Metric + ".txt" 
    QD = open(filename, "a")
...
def calculate_latency(f2, QueueType, str_RTT, RTT, Metric):
    ...
    filename = "./HeteroResults/" + QueueType + "/" + QueueType + str_RTT + "RTT" + Metric + ".txt" 
    QD = open(filename, "a")
...
for i in queue_size:
    for j in round_trip_time:
        if(i=="Threshold100" or i=="Threshold15"):
            f1 = "./HeteroResults/" + i + "/" + i + str(j) + "msRTTQS.txt"
        else: 
            f1 = "./HeteroResults/" + i + "/" + i + str(j) + "RTTQS.txt"
        f2 = "./HeteroResults/" + i + "/" + i + str(j) + "RTTQD.txt"; 
        calculate_QueueingDelay(f1, str(i), str(j), "QD")
        calculate_latency(f2, str(i), str(j), j, "Lat")
        # print(f1, f2, f3, f4)
for i in queue_size:
    for j in round_trip_time:
        f3 = "./HeteroResults/" + i + "/" + i + str(j) + "RTTLat.txt"; 
